      ...
      <div className="mb-8">
        ...
        <div className="bg-white dark:bg-[#121212] rounded-[24px] border border-zinc-200 dark:border-white/5 overflow-hidden divide-y divide-zinc-100 dark:divide-white/5">
            <div className="p-4 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${permissions.notifications ? 'bg-emerald-500/10 text-emerald-500' : 'bg-zinc-100 dark:bg-[#1C1C1E] text-zinc-400'}`}>
                        <Bell size={18} />
                    </div>
                    <div>
                        <h3 className="text-zinc-900 dark:text-white font-medium text-[15px]">Notifications</h3>
                        <p className="text-zinc-500 text-xs">{permissions.notifications ? "Enabled" : "Disabled"}</p>
                    </div>
                </div>
                <button 
                    onClick={() => togglePermission('notifications')}
                    className={`w-12 h-7 rounded-full p-1 transition-colors duration-300 ${permissions.notifications ? 'bg-indigo-600' : 'bg-zinc-200 dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-800'}`}
                >
                     <motion.div 
                        animate={{ x: permissions.notifications ? 20 : 0 }}
                        className={`w-5 h-5 rounded-full shadow-md ${permissions.notifications ? 'bg-white' : 'bg-white dark:bg-zinc-600'}`}
                    />
                </button>
            </div>
            ...
            <div className="p-4 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${permissions.location ? 'bg-sky-500/10 text-sky-500' : 'bg-zinc-100 dark:bg-[#1C1C1E] text-zinc-400'}`}>
                        <MapPin size={18} />
                    </div>
                    <div>
                        <h3 className="text-zinc-900 dark:text-white font-medium text-[15px]">Location Access</h3>
                        <p className="text-zinc-500 text-xs">{permissions.location ? "While In Use" : "Never"}</p>
                    </div>
                </div>
                <button 
                    onClick={() => togglePermission('location')}
                    className={`w-12 h-7 rounded-full p-1 transition-colors duration-300 ${permissions.location ? 'bg-indigo-600' : 'bg-zinc-200 dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-800'}`}
                >
                     <motion.div 
                        animate={{ x: permissions.location ? 20 : 0 }}
                        className={`w-5 h-5 rounded-full shadow-md ${permissions.location ? 'bg-white' : 'bg-white dark:bg-zinc-600'}`}
                    />
                </button>
            </div>
        </div>
